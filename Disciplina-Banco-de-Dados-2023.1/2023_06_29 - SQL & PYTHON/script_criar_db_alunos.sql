...
create table alunos.cursos (
	...
	constraint un_curso  unique (curso)
);
...
create table alunos.situacoes (
	...
	constraint un_situacao  unique (situacao)
);
...
create table alunos.situacoes_sistemicas (
	...
	constraint un_situacao_sistemica   unique (situacao_sistemica)
);
...
create table alunos.linhas_pesquisa (
	...
	constraint un_linha_pesquisa  unique (linha_pesquisa)
);
...
create table alunos.cotas_mec (
	id_cota_mec	serial,
	cota_mec	varchar(200),
	constraint pk_cotas_mec primary key (id_cota_mec),
	constraint un_cota_mec  unique (cota_mec)
);
...
create table alunos.cotas_sistec (
	...
	constraint un_cota_sistec  unique (cota_sistec)
);
